export const meta = {
    title: "Fundamentals of Rendering Data as an SVG Line with D3 and scaleLinear",
    description: "In this lesson we'll start by using `d3-scale` to build up a `scaleLinear` for both our `x` and `y` data points to allow them to map and extrapolate our data correctly to renderable points. Then we'll use `line` from `d3-shape` to build out our `path`. We'll supply it with different sets of data and explore how `range` can effect how our data renders. Finally we'll show off using `curveStep` to see how it changes our graph rendering.",
    createdAt: "2018-03-16T02:15:21.063+00:00"
};
