export const meta = {
    title: "Create an Instagram Press-and-Hold Image Preview Modal with Gesture Actions in React Native",
    description: "In this lesson we'll show how to create an always rendered but invisible modal. Then using `onLayout`, `refs`, and `measure` we can find our action locations. We'll use a `PanResponder` to register presses on an image. When you press and hold we'll use `Animated` to show the modal. With the `onPanResponderMove` we can check if the user has moved their finger onto an action. We'll use `setState` updater function to highlight the action in bold and show how to take action in `onPanResponderRelease`.\n",
    createdAt: "2018-05-21T23:57:16.337173+00:00"
};
