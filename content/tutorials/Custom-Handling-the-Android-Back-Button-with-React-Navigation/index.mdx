export const meta = {
    title: "Custom Handling the Android Back Button with React Navigation",
    description: "In this lesson we'll explore setting up a stack navigator in React Navigation. We'll create a custom component that uses `withNavigation` to allow us to listen to navigation transitions. Using the `BackHandler` from React Native we can ask the screen if custom needs are required when the hardware back button is pressed. Finally we'll prompt the user if they want to navigate.",
    createdAt: "2018-08-07T04:12:39.231+00:00"
};
